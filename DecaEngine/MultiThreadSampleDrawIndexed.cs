...
public class MultiThreadSampleDrawIndexed(GraphicsBackend backend, uint gridSize) : Application(backend)
{
    ...
    private void PopulateInstanceBuffer()
    {
        ...
        ImmediateContext.UpdateBuffer<Matrix4x4>(_instancingBuffer,
            0,
    ...
    private unsafe void RenderSubset(IDeviceContext context, int subset)
    {
        if (_vertexBuffer is null ||
            ...
            _instancingBuffer is null ||
            _indexBuffer is null ||
            _pipelineState is null ||
            _srb is null)
            return;

        context.SetRenderTargets([SwapChain.GetCurrentBackBufferRTV()], SwapChain.GetDepthBufferDSV(), ResourceStateTransitionMode.Transition);
        ...
        context.SetVertexBuffers(0, [_vertexBuffer, _instancingBuffer], [0, 0], ResourceStateTransitionMode.Transition, SetVertexBuffersFlags.Reset);
        context.SetIndexBuffer(_indexBuffer, 0, ResourceStateTransitionMode.Transition);

        var numSubsets = _threads.Count + 1;
        var numInstances = _instanceTransforms.Length;
        var subsetSize = numInstances / numSubsets;
        uint startInst = (uint)(subsetSize * subset);
        uint endInst = (uint)((subset < numSubsets - 1) ? subsetSize * (subset + 1) : numInstances);
        ...
        var drawAttrs = new DrawIndexedAttribs()
        {
            ...
            NumIndices = 36,
            FirstInstanceLocation = startInst,
            NumInstances = endInst,
