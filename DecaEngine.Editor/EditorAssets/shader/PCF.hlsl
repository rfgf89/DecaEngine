                                ...
                                in float2                 f2FilterSize)
{
    ...
    for (int x = StartTexelXY.x; x < EndTexelXY.x; x += 2)
    {
        ...
        for (int y = StartTexelXY.y; y < EndTexelXY.y; y += 2)
        {
            ...
            float2 f2UV_Samp = float2(U0 + dU, V0 + dV);

            float Weight = HorzWeight * VertWeight;
            const float DepthClamp = 1e-8;
            float fDepth = max(fLightSpaceDepth + dot(f2UV_Samp - f2CenterTexel, f2ReceiverPlaneDepthBias), DepthClamp);
            f2UV_Samp *= f4ShadowMapSize.zw;
            #ifdef GLSL
                Sum += tex2DShadowMap.SampleCmp(tex2DShadowMap_sampler, float3(f2UV_Samp, fSlice), fDepth) * Weight;
            #else
                Sum += tex2DShadowMap.SampleCmpLevelZero(tex2DShadowMap_sampler, float3(f2UV_Samp, fSlice), fDepth) * Weight;
            #endif
            TotalWeight += Weight;
        }
    }
